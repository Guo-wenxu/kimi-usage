...
export interface FreshTokenResult {
  token?: string;
  /** True when stored OAuth credentials were wiped after a failed refresh. */
  refreshFailed?: boolean;
}

/**
 * Returns a non-expired access token, refreshing in place when necessary.
 * Missing `expiresAt` (0) is treated as unknown expiry → force refresh.
 */
export async function ensureFreshToken(secrets: vscode.SecretStorage): Promise<FreshTokenResult> {
