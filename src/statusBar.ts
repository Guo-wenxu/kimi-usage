...
import { fmtHours, hoursUntil, PACE_AHEAD_DELTA, weeklyElapsedPct } from './utils';
...
export class StatusBar {
  ...
  private paceEmoji(usedPct: number, elapsedPct: number): string {
    ...
    // Same ahead cutoff as the dashboard; yellow covers slight over-pace before that.
    if (delta > PACE_AHEAD_DELTA) return '🔴';
    if (delta > 0) return '🟡';
    return '🟢';
  }
  ...
  render(state: QuotaState, hasAuth: boolean): void {
    ...
    const resetHours = hoursUntil(state.weeklyResetAt) ?? state.weeklyResetHours;
    if (state.weeklyUsedPct !== null && resetHours !== null && state.weeklyLimit !== null) {
      const usedPct = state.weeklyUsedPct;
      const elapsedPct = weeklyElapsedPct(resetHours);
      ...
      weeklyLine =
        ...
        `Resets in ${fmtHours(Math.max(0, resetHours))}`;
    }

    let windowText = '…';
    const windowIcon = '$(pulse)'; // Rate window icon
    let windowLine = '';
    if (state.windowRemaining !== null && state.windowLimit !== null && state.windowLimit > 0) {
      ...
      const winResetHours = hoursUntil(state.windowResetAt);
      const winReset = winResetHours !== null ? fmtHours(Math.max(0, winResetHours)) : '?';
      windowLine =
