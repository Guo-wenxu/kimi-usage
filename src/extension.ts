...
/** Serializes refresh calls so concurrent timer/command/dashboard polls cannot reorder snapshots. */
    ...
    vscode.commands.registerCommand('kimiUsage.showDashboard', async () => {
      DashboardPanel.show(state, context, usageTracker, {
        ...
        toggleLanguage: async () => {
          const current = resolveLocale(getConfig().language, vscode.env.language);
          const next = current === 'zh-CN' ? 'en' : 'zh-CN';
          await vscode.workspace
            .getConfiguration('kimiUsage')
            .update('language', next, vscode.ConfigurationTarget.Global);
          // onDidChangeConfiguration listener re-renders the dashboard.
        }
      });
      await refresh();
    }),
...
async function resolveToken(): Promise<string | undefined> {
  const oauth = await ensureFreshToken(extensionContext.secrets);
  if (oauth.refreshFailed) {
    vscode.window.showWarningMessage(
      'Kimi OAuth session expired and was cleared. Sign in again, or an API key will be used if configured.'
    );
  }
...
async function refresh(force: boolean = false): Promise<void> {
  const run = refreshGate.then(() => doRefresh(force));
  // Keep the gate alive even if a refresh fails so the next caller still serializes.
  refreshGate = run.catch(() => undefined);
  return run;
}
...
async function promptForApiKey(): Promise<void> {
  ...
  if (!trimmed.startsWith('sk-')) {
    vscode.window.showErrorMessage('Kimi API key should start with "sk-".');
    return;
  }
